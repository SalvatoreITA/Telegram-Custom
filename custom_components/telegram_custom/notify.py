...
class TelegramCustomService(BaseNotificationService):
    ...
    def _detect_media_type(data: dict):
        ...
        # FIX AUDIO: Usa send_voice per i file audio
    ...
    # -------------------------
    # AUTO-DETECT FORMATTAZIONE (Case Insensitive Fix)
    # -------------------------
    def detect_format(self, text: str):
        if not isinstance(text, str): return "markdown"
        # Controllo case-insensitive per i tag HTML
    ...
    async def async_send_message(self, message="", **kwargs):
        ...
        # 1. Rileva Parse Mode (Normalizzato in minuscolo per sicurezza)
        ...
        # CASO SPECIALE: LOCATION (Modificato per inviare anche il testo)
        if media_key == "location":
            ...
            # 1. INVIA LA MAPPA
            # Rimuoviamo message/parse_mode perché send_location non li accetta
            service_data.pop("message", None)
            service_data.pop("parse_mode", None)

            try:
                # Usiamo blocking=True per assicurarci che la mappa parta prima del testo
                await self._hass.services.async_call("telegram_bot", "send_location", service_data, blocking=True)
            except Exception as e:
                _LOGGER.error("TelegramCustom: Errore invio location: %s", e)

            # 2. INVIA IL TESTO (Se presente)
            # Costruiamo il messaggio completo con eventuale titolo
            final_msg = message
            ...
            if final_msg:
                ...
                await self._hass.services.async_call("telegram_bot", "send_message", msg_data, blocking=False)

            # Usciamo perché abbiamo gestito tutto qui
            return

        # CASO MEDIA STANDARD (Photo, Video, Audio)
        elif media_key:
        ...
        # -----------------------------------------------------------
        # PULIZIA DATI E INVIO FINALE (FIX HA 2026.X)
        # -----------------------------------------------------------

        # Filtro chiavi extra per evitare errori di validazione.
        # Rimuoviamo context, metadata, entity_id, push, ecc.
        excluded_keys = [
        ...
        # Gestione Keyboard comuni
        if "inline_keyboard" in data and media_key != "location": service_data["inline_keyboard"] = data["inline_keyboard"]
        reply_markup = data.get("reply_markup")
        if reply_markup and media_key != "location":
            if "inline_keyboard" in reply_markup: service_data["inline_keyboard"] = reply_markup["inline_keyboard"]
        ...
        # Esecuzione con LOG di DEBUG (Utile se qualcosa fallisce)
